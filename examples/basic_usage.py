...
def basic_prediction_example():
    ...
    for model_name, metrics in results.items():
        print(
            f"   {model_name}: RMSE={metrics.get('rmse', 0):.4f}, MAE={metrics.get('mae', 0):.4f}"
        )
...
def multiple_stocks_example():
    ...
    for symbol in symbols:
        ...
        # Select features
        feature_columns = fe.select_features(
            symbol_data_clean, target_column, method="correlation", threshold=0.001
        )

        if not feature_columns:
            feature_columns = [
                col
                for col in symbol_data_clean.columns
                if col not in ["Symbol", "Stock_Symbol"]
                and symbol_data_clean[col].dtype in ["float64", "int64"]
                and "Target_" not in col
            ]
        ...
        try:
            ...
            print(
                f"✅ {symbol}: RMSE={list(model_results.values())[0].get('rmse', 0):.4f}"
            )
...
def classification_example():
    ...
    if target_column not in engineered_data.columns:
        ...
        if returns_col in engineered_data.columns:
            engineered_data[target_column] = (engineered_data[returns_col] > 0).astype(
                int
            )
        else:
