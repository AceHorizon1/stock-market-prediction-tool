...
class HFTransformerPredictor:
    ...
    def prepare_data(
        ...
        for i in range(
            self.context_length, len(scaled_data) - self.prediction_length + 1
        ):
    ...
    def train(
        ...
        if (
            hasattr(self.config, "num_input_channels")
            and self.config.num_input_channels != num_features
        ):
            print(
                f"⚠️ Updating model config: num_input_channels from {self.config.num_input_channels} to {num_features}"
            )
        ...
        print(
            f"📊 Training samples: {len(train_past)}, Validation samples: {len(val_past)}"
        )
...
class HFTransformerStockPredictor:
    ...
    def train_model(
        self, data: pd.DataFrame, target_column: str, epochs: int = 10
    ) -> Dict[str, Any]:
        """Train the model (compatible with existing interface)"""
        results = self.predictor.train(
            data=data, target_column=target_column, epochs=epochs
        )
    ...
    def predict(self, X: pd.DataFrame, model_name: str = None) -> np.ndarray:
        ...
        predictions = self.predictor.predict(data=X, target_column=target_col)
