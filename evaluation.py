...
class ModelEvaluator:
    ...
    def calculate_regression_metrics(
        ...
        metrics["mape_trimmed"] = (
            np.mean(np.abs((y_true - y_pred) / y_true)[np.abs(y_true) > 0.01]) * 100
        )
        ...
        metrics["adjusted_r_squared"] = 1 - (1 - metrics["r_squared"]) * (n - 1) / (
            n - p - 1
        )

        # Information ratio (assuming returns)
        metrics["information_ratio"] = (
            np.mean(y_pred - y_true) / np.std(y_pred - y_true)
        ...
        metrics["sharpe_ratio"] = (
            np.mean(y_pred) / np.std(y_pred) if np.std(y_pred) != 0 else 0
        )
    ...
    def backtest_strategy(
        ...
        if strategy_type == "long_short":
            ...
            df.loc[df["Predictions"] > 0, "Strategy_Returns"] = df.loc[
                df["Predictions"] > 0, "Returns"
            ]
            df.loc[df["Predictions"] < 0, "Strategy_Returns"] = -df.loc[
                df["Predictions"] < 0, "Returns"
            ]
            ...
            df.loc[df["Predictions"] > threshold, "Strategy_Returns"] = df.loc[
                df["Predictions"] > threshold, "Returns"
            ]
    ...
    def plot_backtest_results(
        self, data: pd.DataFrame, backtest_results: Dict[str, float]
    ) -> go.Figure:
        """
        ...
        fig.add_trace(
            go.Scatter(
                x=data.index, y=data["Strategy_Cumulative_Returns"], name="Strategy"
            ),
            row=1,
            col=2,
        )
    ...
    def plot_roc_curve(self, y_true: np.ndarray, y_prob: np.ndarray) -> go.Figure:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=fpr, y=tpr, mode="lines", name=f"ROC Curve (AUC = {auc:.3f})")
        )
        fig.add_trace(
            go.Scatter(
                x=[0, 1],
    ...
    def generate_evaluation_report(
        ...
        if task == "regression":
            ...
            report["metrics"] = self.calculate_classification_metrics(
                y_true, predictions
            )
        ...
        if task == "classification":
            report["plots"]["confusion_matrix"] = self.plot_confusion_matrix(
                y_true, predictions
            )
...
if __name__ == "__main__":
    ...
    data = pd.DataFrame({"Close": prices, "Returns": returns}, index=dates)
