...
class EnhancedDataCollector:
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            logger.warning(
                "Alpha Vantage API key not found. Please set it in config or pass to constructor."
            )
    ...
    def _load_from_cache(
        self, cache_key: str, data_type: str = "raw"
    ...
    def _save_to_cache(
        self, data: pd.DataFrame, cache_key: str, data_type: str = "raw"
    ) -> None:
        """Save data to cache"""
        try:
            cache_path = self._get_cache_path(cache_key, data_type)
    ...
    def _fetch_stock_data_alpha_vantage(
        ...
        try:
            ...
            data.columns = [
                col.split(". ")[-1] if ". " in col else col for col in data.columns
            ]
            ...
            if not isinstance(data.index, pd.DatetimeIndex):
                try:
                    ...
                    logger.warning(
                        f"Could not convert index to datetime for {symbol}: {e}"
                    )
    ...
    async def fetch_multiple_stocks_async(
        ...
        logger.info(
            f"Fetched data for {len(symbols)} symbols in {elapsed_time:.2f} seconds"
        )
    ...
    def fetch_stocks_parallel(self, symbols: List[str], period: str) -> pd.DataFrame:
        ...
        logger.info(
            f"Fetching data for {len(symbols)} symbols using parallel processing"
        )
        start_time = time.time()
        ...
        try:
            ...
            if hasattr(self.ti, function):
                ...
                data, meta_data = method(
                    symbol=symbol, interval=interval, series_type="close"
                )
                return data
            else:
            ...
            logger.error(
                f"Error fetching technical indicator {indicator} for {symbol}: {e}"
            )
            return None
    ...
    def validate_data(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        missing_percentage = (
            (missing_values / total_values) * 100 if total_values > 0 else 100
        )
        ...
        try:
            ...
            for cache_type in ["raw", "processed"]:
                ...
                if cache_dir.exists():
                    ...
                    cache_info[f"{cache_type}_cache_size"] = (
