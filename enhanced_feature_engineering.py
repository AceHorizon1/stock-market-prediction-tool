...
class EnhancedFeatureEngineer:
    ...
    def add_technical_indicators_parallel(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        indicator_functions = [
            ...
            (
                "Stoch_D",
                lambda x: ta.momentum.stoch_signal(x["High"], x["Low"], x["Close"]),
            ),
            (
                "Williams_R",
                lambda x: ta.momentum.williams_r(x["High"], x["Low"], x["Close"]),
            ),
        ...
        with ThreadPoolExecutor(max_workers=config.data.max_workers) as executor:
            futures = [
                executor.submit(calculate_indicator, (name, func))
    ...
    def add_statistical_features_optimized(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in windows:
            # Returns statistics
            df[f"Returns_Mean_{window}d"] = (
                df["Returns"].rolling(window=window, min_periods=1).mean()
            )
            df[f"Returns_Std_{window}d"] = (
                df["Returns"].rolling(window=window, min_periods=1).std()
            )
            df[f"Returns_Skew_{window}d"] = (
                df["Returns"].rolling(window=window, min_periods=1).skew()
            )
            df[f"Returns_Kurt_{window}d"] = (
                df["Returns"].rolling(window=window, min_periods=1).kurt()
            )
            ...
            if "Volume" in df.columns:
                ...
                df[f"Volume_Ratio_{window}d"] = (
                    df["Volume"] / df[f"Volume_Mean_{window}d"]
                )
    ...
    def add_time_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df["Is_Quarter_End"] = (
            (df.index.month % 3 == 0) & (df.index.day >= 25)
        ).astype(int)
    ...
    def create_target_variables(
        self, data: pd.DataFrame, horizons: List[int]
    ) -> pd.DataFrame:
        """
        ...
        if method == "correlation":
            ...
            if y.dtype == "object" or len(y.unique()) < 10:
                ...
                rf = RandomForestClassifier(
                    n_estimators=100, random_state=config.model.random_state
                )
            else:
                # Regression
            ...
            for method_name in methods:
                try:
                    selected = self.select_features_advanced(
                        data, target_column, method_name, n_features
                    )
            ...
            if len(selected_features) < n_features // 2:
                ...
                selected_features = self.select_features_advanced(
                    data, target_column, "correlation", n_features
                )
    ...
    def scale_features(
        ...
        if fit:
            ...
            if method in self.scalers:
                df[feature_columns] = self.scalers[method].transform(
                    df[feature_columns]
                )
            else:
    ...
    def get_feature_summary(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "feature_categories": {
                ...
                "statistical_features": len(
                    [
                        col
                        for col in data.columns
                        if any(
                            stat in col
                            for stat in ["Returns_", "Volume_", "Price_", "Volatility_"]
                        )
                    ]
                ),
                "microstructure_features": len(
                    [
                        col
                        for col in data.columns
                        if any(
                ...
                "target_variables": len(
                    [col for col in data.columns if "Target_" in col]
                ),
            },
