...
class StockPredictionGUI:
    ...
    def create_layout(self):
        ...
        data_tab = [
            ...
            [sg.Radio('Upload CSV/Excel File', 'DATASRC', key='-FILE-', default=True, font=('Arial', 11)),
             sg.Radio('Fetch Online Data', 'DATASRC', key='-ONLINE-', font=('Arial', 11))],
            [sg.Text('File Path:'), sg.Input(key='-FILEPATH-', size=(50, 1)), 
             sg.FileBrowse('Browse', file_types=(('CSV/Excel Files', '*.csv;*.xlsx'),))],
            [sg.Text('Stock Symbols (comma separated):'), sg.Input(key='-SYMBOLS-', size=(40, 1), 
             default_text='AAPL, MSFT, GOOGL')],
            [sg.Button('Load Data', size=(15, 1), button_color=('white', 'blue'))],
            [sg.HorizontalSeparator()],
            [sg.Text('Data Information', font=('Arial', 12, 'bold'))],
            [sg.Multiline(size=(80, 10), key='-DATA_INFO-', disabled=True, background_color='lightgray')]
        ]
        ...
        features_tab = [
            ...
            [sg.HorizontalSeparator()],
            [sg.Button('Engineer Features', size=(15, 1), button_color=('white', 'green'))],
            [sg.HorizontalSeparator()],
            [sg.Text('Feature Information', font=('Arial', 12, 'bold'))],
            [sg.Multiline(size=(80, 8), key='-FEATURE_INFO-', disabled=True, background_color='lightgray')],
            [sg.Text('Sample Engineered Data (First 5 rows):', font=('Arial', 11, 'bold'))],
            [sg.Table(values=[], headings=[], auto_size_columns=True, 
                     display_row_numbers=True, num_rows=5, key='-FEATURE_TABLE-', 
                     size=(80, 10), background_color='white', text_color='black')]
        ]
        ...
        model_tab = [
            ...
            [sg.Text('Model Type:'), sg.Combo(['ensemble', 'tree', 'linear', 'neural', 'deep', 'hf_transformer'], 
             default_value='ensemble', key='-MODEL_TYPE-', size=(15, 1))],
            [sg.Text('Prediction Task:'), sg.Combo(['regression', 'classification'], 
             default_value='regression', key='-TASK-', size=(15, 1))],
            [sg.Text('Target Horizon (days):'), sg.Combo([1, 3, 5, 10, 20], 
             default_value=1, key='-HORIZON-', size=(15, 1))],
            [sg.Button('Train Model', size=(15, 1), button_color=('white', 'orange'))],
            [sg.HorizontalSeparator()],
            [sg.Text('Training Progress:', font=('Arial', 11, 'bold'))],
            [sg.Multiline(size=(80, 10), key='-TRAINING_INFO-', disabled=True, background_color='lightgray')]
        ]
        ...
        results_tab = [
            ...
            [sg.Multiline(size=(80, 8), key='-RESULTS_INFO-', disabled=True, background_color='lightgray')],
            [sg.Text('Predictions Table:', font=('Arial', 11, 'bold'))],
            [sg.Table(values=[], headings=[], auto_size_columns=True, 
                     display_row_numbers=True, num_rows=10, key='-PREDICTIONS_TABLE-', 
                     size=(80, 12), background_color='white', text_color='black')],
            [sg.Canvas(key='-CANVAS-', size=(800, 600))]
        ]
        
        # Main layout with tabs
        layout = [
            [sg.Text('Stock Market Prediction Tool', font=('Arial', 18, 'bold'), 
    ...
    def load_data(self, values):
        ...
        try:
            if values['-FILE-'] and values['-FILEPATH-']:
                ...
                # Load from online
                symbols = [s.strip().upper() for s in values['-SYMBOLS-'].split(',') if s.strip()]
                ...
                info_text = "❌ Please select a data source and provide the required input."
    ...
    def engineer_features(self):
        ...
        try:
            ...
            # Categorize features
            feature_categories = {
                'Technical Indicators': [col for col in self.engineered_data.columns if any(x in col for x in ['RSI', 'MACD', 'BB', 'SMA', 'EMA'])],
                'Statistical Features': [col for col in self.engineered_data.columns if any(x in col for x in ['Rolling', 'Std', 'Mean', 'Skew', 'Kurt'])],
                'Target Variables': [col for col in self.engineered_data.columns if 'Target_' in col],
                'Time Features': [col for col in self.engineered_data.columns if any(x in col for x in ['Day', 'Month', 'Year', 'Quarter'])],
                'Other Features': [col for col in self.engineered_data.columns if not any(x in col for x in ['RSI', 'MACD', 'BB', 'SMA', 'EMA', 'Rolling', 'Std', 'Mean', 'Skew', 'Kurt', 'Target_', 'Day', 'Month', 'Year', 'Quarter'])]
            }
    ...
    def train_model(self, values):
        ...
        try:
            if self.engineered_data is None or self.engineered_data.empty:
                return False, "❌ No engineered data available. Please engineer features first."
            ...
            if model_type == 'hf_transformer':
                training_info = f"🚀 Training Hugging Face Transformer (PatchTST) model...\n"
                training_info += f"📊 Target: {target_column}\n"
                training_info += f"📈 Data shape: {data_clean.shape}\n"
                training_info += f"⏱️  Training started at: {datetime.now().strftime('%H:%M:%S')}\n"
            ...
            training_info += f"✅ Training completed at: {datetime.now().strftime('%H:%M:%S')}\n"
    ...
    def show_results(self):
        ...
        try:
            if self.model_results is None:
                return False, "❌ No model results available. Please train a model first."
    ...
    def generate_plots(self, window):
        ...
        try:
            ...
            fig.suptitle('Stock Market Prediction Results', fontsize=14, fontweight='bold')
            ...
            if 'predictions' in self.model_results:
                predictions = self.model_results['predictions']
                axes[0, 1].hist(predictions, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
                axes[0, 1].set_xlabel('Predicted Values', fontsize=10)
                axes[0, 1].set_ylabel('Frequency', fontsize=10)
                axes[0, 1].set_title('Prediction Distribution', fontsize=12, fontweight='bold')
                axes[0, 1].grid(True, alpha=0.3)
            
            # Plot 3: Feature Importance (if available)
            if 'feature_importance' in self.model_results and self.model_results['feature_importance'] is not None:
                feature_importance = self.model_results['feature_importance']
                if len(feature_importance) > 0:
                    top_features = feature_importance.head(10)
                    y_pos = np.arange(len(top_features))
                    axes[1, 0].barh(y_pos, top_features.values, color='lightgreen', edgecolor='black')
                    axes[1, 0].set_yticks(y_pos)
                    axes[1, 0].set_yticklabels([f[:20] + '...' if len(f) > 20 else f for f in top_features.index], fontsize=8)
            ...
            if 'predictions' in self.model_results and self.engineered_data is not None:
                predictions = self.model_results['predictions']
                test_indices = self.model_results.get('test_indices', range(len(predictions)))
                
                if len(test_indices) == len(predictions):
                    axes[1, 1].plot(test_indices, predictions, label='Predictions', alpha=0.7, linewidth=1)
                    if 'y_test' in self.model_results:
                        y_test = self.model_results['y_test']
                        axes[1, 1].plot(test_indices, y_test, label='Actual', alpha=0.7, linewidth=1)
                    axes[1, 1].set_xlabel('Time Index', fontsize=10)
                    axes[1, 1].set_ylabel('Values', fontsize=10)
                    axes[1, 1].set_title('Predictions Over Time', fontsize=12, fontweight='bold')
                    axes[1, 1].legend()
                    axes[1, 1].grid(True, alpha=0.3)
                else:
                    axes[1, 1].text(0.5, 0.5, 'Time series data\nnot available', ha='center', va='center', transform=axes[1, 1].transAxes)
    ...
    def run(self):
        ...
        window = sg.Window('Stock Market Prediction Tool', layout, 
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Exit':
                ...
                if success:
                    sg.popup('Success!', 'Data loaded successfully.', auto_close=True, auto_close_duration=2)
                ...
                if success:
                    ...
                    self.update_table(window, '-FEATURE_TABLE-', self.engineered_data, max_rows=5)
                    sg.popup('Success!', 'Features engineered successfully.', auto_close=True, auto_close_duration=2)
            
            elif event == 'Train Model':
                success, message = self.train_model(values)
                window['-TRAINING_INFO-'].update(message)
                if success:
                    sg.popup('Success!', 'Model trained successfully.', auto_close=True, auto_close_duration=2)
                ...
                if success:
                    ...
                    if 'predictions' in self.model_results:
                        pred_df = pd.DataFrame({
                            'Predictions': self.model_results['predictions'],
                            'Actual': self.model_results.get('y_test', [None] * len(self.model_results['predictions']))
                        })
                        self.update_table(window, '-PREDICTIONS_TABLE-', pred_df, max_rows=10)
                    sg.popup('Success!', 'Results displayed successfully.', auto_close=True, auto_close_duration=2)
