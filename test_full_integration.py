...
def test_imports():
    ...
    try:
        from hf_transformer_models import (
            HFTransformerPredictor,
            HFTransformerStockPredictor,
        )
...
def test_hf_transformer_basic(data):
    ...
    try:
        ...
        if len(test_data) < 200:
            print(
                f"⚠️  Only {len(test_data)} samples available. Need at least 200 for good results."
            )
...
def test_integration_with_existing_code(engineered_data):
    ...
    try:
        ...
        predictor = AdvancedStockPredictor(
            model_type="hf_transformer", task="regression"
        )
...
def main():
    ...
    print(
        f"{'✅' if data is not None else '❌'} Data Collection: {'Passed' if data is not None else 'Failed'}"
    )
    print(
        f"{'✅' if engineered_data is not None else '❌'} Feature Engineering: {'Passed' if engineered_data is not None else 'Failed'}"
    )
    print(
        f"{'✅' if hf_success else '⚠️ '} HF Transformer Basic: {'Passed' if hf_success else 'Skipped/Failed'}"
    )
    print(
        f"{'✅' if integration_success else '⚠️ '} Integration Test: {'Passed' if integration_success else 'Skipped/Failed'}"
    )
    ...
    if hf_success:
        ...
        print(
            "  2. Train models using AdvancedStockPredictor with model_type='hf_transformer'"
        )
