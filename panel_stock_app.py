...
class PanelStockApp:
    ...
    def _create_ui(self):
        ...
        # Sidebar controls
        self.symbols_input = pn.widgets.TextInput(
            name="Stock Symbols",
            value="AAPL, MSFT, GOOGL",
            placeholder="Enter symbols separated by commas",
        )
        self.period_select = pn.widgets.Select(
            name="Time Period", options=["1y", "2y", "5y", "10y", "max"], value="2y"
        )
        self.load_button = pn.widgets.Button(name="🚀 Load Data", button_type="primary")
        self.download_button = pn.widgets.Button(
            name="📥 Download CSV", button_type="default"
        )

        # Feature engineering controls
        self.technical_cb = pn.widgets.Checkbox(name="Technical Indicators", value=True)
        self.statistical_cb = pn.widgets.Checkbox(
            name="Statistical Features", value=True
        )
        self.microstructure_cb = pn.widgets.Checkbox(
            name="Market Microstructure", value=True
        )
        self.time_cb = pn.widgets.Checkbox(name="Time Features", value=True)
        self.engineer_button = pn.widgets.Button(
            name="⚙️ Engineer Features", button_type="primary"
        )
        ...
        self.task_select = pn.widgets.Select(
            name="Task", options=["regression", "classification"], value="regression"
        )
        self.horizon_select = pn.widgets.Select(
            name="Target Horizon", options=[1, 3, 5, 10, 20], value=1
        )
        self.train_button = pn.widgets.Button(
            name="🎯 Train Model", button_type="primary"
        )
        ...
        main_content = pn.Column(
            ...
            pn.Tabs(
                (
                    "📊 Data Overview",
                    pn.Column(self.info_pane, self.price_chart, self.data_table),
                ),
                (
                    "🔧 Feature Engineering",
                    pn.Column(self.feature_chart, self.features_table),
                ),
                ("🤖 Model Training", pn.Column(self.results_chart)),
                (
    ...
    def _load_data(self, event):
        ...
        try:
            ...
            symbols = [
                s.strip().upper()
                for s in self.symbols_input.value.split(",")
                if s.strip()
            ]
            ...
            if self.data is not None and not self.data.empty:
                ...
                self.status_pane.object = (
                    f"### Status: ✅ Data loaded successfully! {len(self.data)} rows"
                )
    ...
    def _engineer_features(self, event):
        ...
        if self.data is None or self.data.empty:
            self.status_pane.object = (
                "### Status: ❌ No data loaded. Please load data first."
            )
            return
        ...
        try:
            ...
            self.status_pane.object = (
                f"### Status: ❌ Error engineering features: {str(e)}"
            )
    ...
    def _train_model(self, event):
        ...
        if self.engineered_data is None or self.engineered_data.empty:
            self.status_pane.object = (
                "### Status: ❌ No engineered features. Please engineer features first."
            )
            return
        ...
        try:
            ...
            self.predictor = AdvancedStockPredictor(
                model_type=self.model_type_select.value, task=self.task_select.value
            )

            # Create target column
            target_column = (
                f"Target_Return_{self.horizon_select.value}d"
                if self.task_select.value == "regression"
                else f"Target_Direction_{self.horizon_select.value}d"
            )
    ...
    def _create_feature_chart(self):
        ...
        try:
            ...
            numeric_cols = self.engineered_data.select_dtypes(
                include=[np.number]
            ).columns
            corr_matrix = self.engineered_data[numeric_cols].corr()

            fig = px.imshow(corr_matrix, title="Feature Correlation Matrix", height=400)
