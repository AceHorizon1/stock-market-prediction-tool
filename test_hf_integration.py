...
def test_hf_transformer_integration():
    ...
    if len(prepared_data) < 200:
        print(
            f"⚠️ Warning: Only {len(prepared_data)} samples available. Need at least 200 for good results."
        )
    ...
    print("\n📝 Next steps:")
    print(
        "   1. Experiment with different model types (Autoformer, TimeSeriesTransformer)"
    )
