...
class StockPredictionApp:
    ...
    def run(self):
        ...
        st.markdown(
            "A comprehensive AI-powered tool for predicting stock market movements"
        )
    ...
    def sidebar(self):
        ...
        symbols_input = st.sidebar.text_input(
            "Stock Symbols (comma-separated)", value=", ".join(default_symbols)
        )
        symbols = [s.strip().upper() for s in symbols_input.split(",") if s.strip()]

        # Time period
        period = st.sidebar.selectbox(
            "Time Period", options=["1y", "2y", "5y", "10y", "max"], index=2
        )

        # Include additional data
        st.sidebar.info(
            "💡 Note: External APIs (market/economic data) are temporarily disabled for demo purposes"
        )
        include_market_data = st.sidebar.checkbox(
            "Include Market Data", value=False, disabled=True
        )
        include_economic_data = st.sidebar.checkbox(
            "Include Economic Data", value=False, disabled=True
        )
        ...
        if st.sidebar.button("Load Data", type="primary"):
            with st.spinner("Loading data..."):
                self.load_data(
                    symbols, period, include_market_data, include_economic_data
                )
        ...
        if st.session_state.get("data_loaded", False):
            ...
            task = st.sidebar.selectbox(
                "Prediction Task", options=["regression", "classification"], index=0
            )

            target_horizon = st.sidebar.selectbox(
                "Prediction Horizon (days)", options=[1, 3, 5, 10, 20], index=0
            )
    ...
    def load_data(self, symbols, period, include_market_data, include_economic_data):
        ...
        try:
            ...
            st.success(
                f"✅ Data loaded successfully! Shape: {self.engineered_data.shape}"
            )
            ...
            with st.expander("Data Information"):
                ...
                st.write(
                    f"**Date Range:** {self.engineered_data.index.min()} to {self.engineered_data.index.max()}"
                )
    ...
    def train_model(self, model_type, task, target_horizon):
        ...
        try:
            ...
            if self.engineered_data is None or self.engineered_data.empty:
                st.error(
                    "No engineered data available. Please load and engineer features first."
                )
                return
            ...
            if model_type == "hf_transformer":
                st.warning(
                    "⚠️ WARNING: HF Transformer can be unstable and may cause crashes. Consider using 'ensemble' or 'tree' models for more reliable results."
                )
                st.info(
                    "🤖 Using Hugging Face Transformer (PatchTST). Training with minimal settings for stability."
                )

                # Check data size
                if len(X_train) < 200:
                    st.error(
                        f"❌ Insufficient data for HF transformer. Need at least 200 samples, got {len(X_train)}. Please load more data or use a different model."
                    )
                    return
                ...
                try:
                    with st.spinner(
                        "Training HF transformer (this may take a while and could be unstable)..."
                    ):
                    ...
                    st.info(
                        "💡 Tip: Try using 'ensemble' or 'tree' model types instead, or reduce your data size."
                    )
                    return
                except Exception as e:
                    st.error(f"❌ Error training HF transformer: {str(e)}")
                    st.warning(
                        "⚠️ HF transformer crashed. This is a known issue with the current implementation."
                    )
                    st.info(
                        "💡 Recommendation: Use 'ensemble' or 'tree' model types for more stable results."
                    )
                    return
            else:
                # Train models
    ...
    def main_content(self):
        ...
        if self.engineered_data is None or self.engineered_data.empty:
            st.info(
                "📊 Data loaded. Please wait for feature engineering to complete, or engineer features manually."
            )
            return
    ...
    def show_data_overview(self):
        ...
        if self.engineered_data is None or self.engineered_data.empty:
            st.warning(
                "⚠️ No engineered data available. Please load and engineer features first."
            )
            return
        ...
        with col3:
            st.metric(
                "Date Range",
                f"{self.engineered_data.index[0].date()} to {self.engineered_data.index[-1].date()}",
            )
        ...
        if "Close" in self.engineered_data.columns:
            ...
            if "Stock_Symbol" in self.engineered_data.columns:
                ...
                for symbol in symbols[:3]:  # Show first 3 symbols
                    symbol_data = self.engineered_data[
                        self.engineered_data["Stock_Symbol"] == symbol
                    ]
        ...
        with col1:
            ...
            if self.engineered_data is not None:
                ...
                if feature_columns and all(
                    col in latest_data.columns for col in feature_columns
                ):
                    ...
                    if st.button("Predict Next Day"):
                        try:
                            ...
                            st.success(
                                f"Predicted Return: {prediction[0]:.4f} ({prediction[0]*100:.2f}%)"
                            )

                            # Interpret prediction
                            if prediction[0] > 0:
                                st.info(
                                    "📈 Bullish prediction - Stock expected to rise"
                                )
                            else:
                                st.info(
                                    "📉 Bearish prediction - Stock expected to fall"
                                )
